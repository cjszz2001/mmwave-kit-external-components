...
SceneModeSelect = ti6342_ns.class_("SceneModeSelect", select.Select)
UnmanTimeSelect = ti6342_ns.class_("UnmanTimeSelect", select.Select)
ExistenceBoundarySelect = ti6342_ns.class_("ExistenceBoundarySelect", select.Select)
MotionBoundarySelect = ti6342_ns.class_("MotionBoundarySelect", select.Select)
...
async def to_code(config):
    ...
    if scenemode_config := config.get(CONF_SCENE_MODE):
        ...
        cg.add(ti6342_component.set_scene_mode_select(s))
    if unmantime_config := config.get(CONF_UNMAN_TIME):
        ...
        await cg.register_parented(s, config[CONF_TI6342_ID])
        cg.add(ti6342_component.set_unman_time_select(s))
    if existence_boundary_config := config.get(CONF_EXISTENCE_BOUNDARY):
        ...
        await cg.register_parented(s, config[CONF_TI6342_ID])
        cg.add(ti6342_component.set_existence_boundary_select(s))
    if motion_boundary_config := config.get(CONF_MOTION_BOUNDARY):
        ...
        await cg.register_parented(s, config[CONF_TI6342_ID])
        cg.add(ti6342_component.set_motion_boundary_select(s))
